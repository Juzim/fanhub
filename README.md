...
Интерфейс переключается между русским и казахским через стандартный
Django-механизм i18n (переключатель RU/ҚАЗ в шапке сайта и на экране входа).
Выбор языка хранится в сессии/cookie пользователя.

**Что переведено:** структурные элементы интерфейса — меню, шапка, кнопки,
заголовки разделов, формы входа/регистрации (46 строк, `locale/kk/LC_MESSAGES/django.po`).

**Что НЕ переведено:** реальный контент из базы данных — названия новостей,
тексты статей, названия клубов и т.д. Это осознанное ограничение: сами
новости и результаты матчей — реальные данные с казахстанских спортивных
сайтов на русском языке (см. раздел «Реальные данные КПЛ» ниже); чтобы
показывать их и на казахском, потребовался бы либо ручной перевод каждой
записи, либо подключение платного переводческого API — этого в проекте
осознанно нет, чтобы не переходить в разряд "не бесплатно".

Если захотите расширить перевод на другие страницы (профиль, аналитика,
магазин мерча и т.д.): оберните нужный текст в шаблоне в `{% trans "..." %}`
(не забыв `{% load i18n %}` в начале файла) и добавьте пару
`msgid "..." / msgstr "..."` в `locale/kk/LC_MESSAGES/django.po`.

Компиляция перевода (`.po` → `.mo`) происходит **автоматически** при каждом
запуске контейнера (`entrypoint.sh` вызывает `compilemessages`, для чего в
`Dockerfile` установлен пакет `gettext`). Руками ничего компилировать не
нужно, если работаете через Docker Compose.

Если запускаете без Docker (venv/PyCharm) — потребуется установить gettext
у себя в системе (Windows: https://mlocati.github.io/articles/gettext-iconv-windows/,
Mac: `brew install gettext`, Linux: `apt install gettext`) и один раз выполнить:
```bash
